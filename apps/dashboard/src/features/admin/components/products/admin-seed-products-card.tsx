...
import { adminSeedProductsToVendorAction } from "../../actions";
import { parseProductFile } from "../../../vendor-products/import/parse-product-file";
import type {
...
import type { AdminVendorOption } from "../shared/admin-types";

type AdminSeedProductsCardProps = {
...
export function AdminSeedProductsCard({ categories, vendors, onImportComplete }: AdminSeedProductsCardProps) {
  ...
  async function handleImport() {
    if (!selectedVendorId) {
      setImportResult({
        success: false,
        error: "يرجى اختيار الصيدلية المستهدفة أولًا.",
        totalRows: clientValidation?.totalRows ?? 0,
        insertedCount: 0,
        updatedCount: 0,
        failedCount: clientValidation?.totalRows ?? 0,
        errors: [],
      });
      return;
    }
    ...
    try {
      const result = await adminSeedProductsToVendorAction({
      ...
      setImportResult({
        success: false,
        error: error instanceof Error ? error.message : "تعذر تنفيذ إضافة المنتجات الآن.",
  ...
  return (
    <Card className="medical-panel">
      <div className="split-actions">
        <div>
          <h3 className="order-card-title">إضافة منتجات لصيدلية</h3>
          <p className="muted order-card-subtitle">
            اختر صيدلية معتمدة ثم ارفع ملف CSV أو XLSX بالأعمدة: name, category_slug, price, stock_quantity, barcode.
          </p>
        </div>
      </div>

      <div className="field">
        <label htmlFor="admin-seed-vendor">الصيدلية المستهدفة</label>
        <select
      ...
      >
        ...
        <span className="muted">ندعم ملفات CSV و XLSX مع معاينة أول 5 صفوف قبل التنفيذ.</span>
      ...
      <div className="inline-actions" style={{ justifyContent: "space-between" }}>
        <div className="muted">
          {isParsing
            ? "جارٍ تحليل الملف..."
            : clientValidation
              ? `${clientValidation.totalRows} صفوف جاهزة للمراجعة قبل الإضافة.`
              : "لن تكون الصورة مطلوبة هنا، وسيُستخدم fallback العام حسب الباركود عند توفره."}
        </div>
        <div className="inline-actions">
          <Button type="button" className="secondary-button" onClick={resetImportState} disabled={isParsing || isImporting}>
            مسح
          </Button>
          <Button type="button" onClick={() => void handleImport()} disabled={!hasReadyRows || !selectedVendorId || isParsing || isImporting}>
            {isImporting ? "جارٍ الإضافة..." : "إضافة المنتجات"}
          </Button>
        </div>
      </div>

      {vendors.length === 0 ? <p className="danger">لا توجد صيدليات نشطة ومعتمدة متاحة حاليًا لهذا الإجراء.</p> : null}
        ...
        <div className="stack compact-stack">
          ...
          <div className="stack compact-stack">
            <div>
              ...
              <p className="muted order-card-subtitle">تعرض هذه المعاينة القيم المقروءة مباشرة من الملف قبل إرسالها إلى الخادم.</p>
            </div>
            <div className="import-preview-table">
              ...
              {previewRows.map((row) => (
                <div key={`admin-preview-row-${row.rowNumber}`} className="import-preview-row">
      ...
      {clientErrors.length > 0 ? (
        <div className="stack compact-stack">
          <h4 className="order-card-title" style={{ fontSize: "var(--font-size-heading-md)" }}>
            أخطاء تحتاج تعديلًا قبل الإضافة
          </h4>
          <div className="import-error-list">
            {clientErrors.map((error) => (
              <p key={`admin-client-error-${error.rowNumber}-${error.field}-${error.message}`} className="danger" style={{ margin: 0 }}>
          ...
          <p className={importResult.success ? "success" : "danger"} style={{ margin: 0 }}>
            {importResult.success
              ? importResult.insertedCount > 0 || importResult.updatedCount > 0
                ? "اكتملت إضافة المنتجات إلى الصيدلية المحددة بنجاح."
                : importResult.error ?? "اكتمل الفحص لكن لم يتم إجراء أي تغييرات."
              : importResult.error ?? "تعذر تنفيذ إضافة المنتجات الآن."}
          </p>
          ...
          {importResult.errors.length > 0 ? (
            <div className="import-error-list">
              {importResult.errors.slice(0, 15).map((error) => (
                <p key={`admin-server-error-${error.rowNumber}-${error.field}-${error.message}`} className="danger" style={{ margin: 0 }}>
                  الصف {error.rowNumber}، {formatImportField(error.field)}: {error.message}
                </p>
              ))}
            </div>
