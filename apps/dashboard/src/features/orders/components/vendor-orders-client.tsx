...
const activeVendorStatuses = new Set([
  "accepted",
  "preparing",
  "ready_for_pickup",
]);
...
function readSingle<T extends Record<string, unknown>>(
...
async function loadVendorOrdersData(
  page: number,
  statusFilter: VendorOrderStatusFilter,
): Promise<VendorOrderData> {
  const supabase = getSupabaseBrowserClient();
  const { data: vendorId, error: vendorError } =
  ...
  const [{ data, error, count }, allCountResult, ...statusCountResults] =
    await Promise.all([query, ...countQueries]);
  if (error) {
    throw error;
  }

  const statusCounts = statusCountResults.reduce<
    Record<VendorOrderStatusFilter, number>
  >(
    (accumulator, result, index) => {
      accumulator[vendorOrderStatuses[index]] = result.count ?? 0;
      return accumulator;
    },
    { all: allCountResult.count ?? 0 } as Record<
  ...
  return {
    ...
    orders: buildPaginatedResult(rows, count, {
      page,
...
export function VendorOrdersClient({
  ...
  const [statusFilter, setStatusFilter] = useState<VendorOrderStatusFilter>(
    initialStatusFilter &&
      vendorOrderStatuses.includes(
        initialStatusFilter as (typeof vendorOrderStatuses)[number],
      )
  ...
  const newOrders = useMemo(
    () => filteredOrders.filter((order) => order.orderStatus === "placed"),
    [filteredOrders],
  );
  ...
  async function updateVendorOrderStatus(orderId: string, nextStatus: string) {
    ...
    const previousOrder = data.orders.rows.find(
    ...
    return (
      <Card className="medical-panel">
        <EmptyState
          ...
          message="ستظهر طلبات المتجر هنا بمجرد أن يبدأ الزبائن في الشراء."
        />
      </Card>
    );
  }
    ...
    <div className="stack">
      ...
      <Card className="medical-panel">
        ...
        <div className="filter-chip-row">
          <button
            type="button"
            className={`filter-chip ${statusFilter === "all" ? "filter-chip-active" : ""}`.trim()}
      ...
      <PaginationControls
        ...
        onNext={() =>
          setPage((current) => Math.min(data.orders.pageCount, current + 1))
        }
      />
      ...
      <section className="stack">
        <div>
          ...
          <p className="muted order-card-subtitle">
            طلبات الزبائن بحالة placed وتحتاج قبول الطلب أو رفضه من الصيدلية.
          </p>
        </div>
        {newOrders.length === 0 ? (
          <Card className="medical-panel">
            <EmptyState
              ...
              message="ستظهر هنا الطلبات الجديدة فور إنشائها من الزبون."
            />
          </Card>
        ) : null}
      ...
      <section className="stack">
        <div>
          ...
          <p className="muted order-card-subtitle">
            طلبات مقبولة أو قيد التحضير أو جاهزة للاستلام قبل انتقالها إلى
            السائق.
          </p>
        </div>
        {activeOrders.length === 0 ? (
          <Card className="medical-panel">
            <EmptyState
              ...
              message="بعد قبول الطلب سيظهر هنا حتى يصبح جاهزًا للاستلام."
            />
          </Card>
        ) : null}
        <section className="detail-grid">
        ...
        {deliveryOrders.length === 0 ? (
          <Card className="medical-panel">
            <EmptyState
              ...
              message="ستظهر هنا الطلبات بعد إسنادها للسائق."
            />
          </Card>
        ) : null}
        <section className="detail-grid">
      ...
      <Table
        ...
        emptyMessage={
          statusFilter === "all"
            ? "لا توجد طلبات مكتملة أو مسندة بعد."
            : "لا توجد طلبات للمتجر في هذه المرحلة حاليًا."
        }
      />
    </div>
  );
...
function PaginationControls({
  ...
  return (
    <Card className="medical-panel">
      <div className="split-actions">
        <p className="muted">
          الإجمالي: {totalCount} · الصفحة {page} من {pageCount}
        </p>
        <div className="inline-actions">
          <button
