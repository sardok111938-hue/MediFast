...
} from "../prescriptions/prescription-requests";
import {
...
function formatPrescriptionStatusLabel(
  status: CustomerPrescriptionRequest["status"],
) {
  switch (status) {
    case "pending":
      return "قيد المراجعة";
...
function formatAvailabilityLabel(
  status: CustomerPrescriptionQuoteItem["availabilityStatus"],
) {
  switch (status) {
    case "available":
      return "متوفر";
...
export default function OrderHistoryScreen() {
  ...
  const [prescriptions, setPrescriptions] = useState<
    CustomerPrescriptionRequest[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [respondingQuoteId, setRespondingQuoteId] = useState<string | null>(
    null,
  );
  const [creatingOrderQuoteId, setCreatingOrderQuoteId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const prescriptionsChannel = subscribeToCustomerPrescriptionRequests(
      customerId,
      () => {
        void loadOrders("refresh");
      },
  ...
  return (
    <Screen title="" subtitle="">
      <View style={styles.floatingHeader}></View>
      <ScrollView
      ...
      >
        {loading ? (
          ...
          />
        ) : activeOrders.length === 0 && visiblePrescriptions.length === 0 ? (
          <EmptyCard
          ...
          <>
              ...
              <>
                ...
                {visiblePrescriptions.map((prescription) => (
                  <ListCard
                    ...
                    badge={
                      <StatusBadge
                        label={formatPrescriptionStatusLabel(
                          prescription.status,
                        )}
                  ...
                  >
                    {prescription.vendorNote &&
                    !prescription.quote?.convertedOrderId ? (
                      <View style={styles.prescriptionReply}>
                        <Text style={styles.prescriptionReplyLabel}>
                          رد الصيدلية
                        </Text>
                        <Text style={styles.prescriptionReplyText}>
                          {prescription.vendorNote}
                        </Text>
                      </View>
                    ) : (
                    ...
                    {prescription.quote ? (
                      <View style={styles.quoteBox}>
                        <View style={styles.quoteHeader}>
                          <StatusBadge
                            label={formatQuoteStatusLabel(
                              prescription.quote.status,
                            )}
                          ...
                          {prescription.quote.items.map((item) => (
                            ...
                            >
                              ...
                              <Text style={styles.quoteItemMeta}>
                                ...
                                {formatCustomerCurrency(item.unitPrice)}
                              </Text>
                              {item.note ? (
                        ...
                        <View style={styles.quoteSubtotalRow}>
                          <Text style={styles.quoteSubtotalLabel}>
                            الإجمالي الفرعي
                          </Text>
                          <Text style={styles.quoteSubtotalValue}>
                            {formatCustomerCurrency(
                              prescription.quote.subtotal,
                            )}
                          </Text>
                        </View>
                        ...
                        {prescription.quote.status === "sent" ? (
                          <View style={styles.quoteActions}>
                            <PrimaryButton
                              label={
                                respondingQuoteId === prescription.quote.id
                                  ? "جارٍ القبول..."
                                  : "قبول العرض"
                              }
                              onPress={() =>
                                void handleQuoteResponse(
                                  prescription.quote!.id,
                                  "accepted",
                                )
                              }
                        ...
                        {prescription.quote.status === "accepted" ? (
                          <View style={styles.quoteActions}>
                            {prescription.quote.convertedOrderId ? (
                              ...
                              <>
                                <HelperText tone="success">
                                  تم قبول العرض. يمكنك الآن تأكيد الطلب والدفع
                                  عند الاستلام.
                                </HelperText>

                                <PrimaryButton
                                  label={
                                    creatingOrderQuoteId ===
                                    prescription.quote.id
                                      ? "جارٍ إنشاء الطلب..."
                                      : "تأكيد الطلب"
                                  }
                                  onPress={() =>
                                    void handleCreateOrderFromQuote(
                                      prescription.quote!.id,
                                    )
                                  }
                                  disabled={
                    ...
                    {quoteMessage ? (
                      <HelperText
                        tone={
                          quoteMessage.includes("تعذر") ? "danger" : "success"
                        }
                      >
                        {quoteMessage}
                      </HelperText>
            ...
            {activeOrders.map((order) => {
              const isPrescriptionOrder = Boolean(order.prescriptionQuoteId);
              ...
              return (
                ...
                >
                  <Text style={styles.metaText}>
                    {isPrescriptionOrder ? "طلب وصفة طبية" : "طلب عادي"}
                  </Text>
